...
if __name__ == "__main__":
    ...
    s = open('ChinhTri_test/XH_NLD_T_ (8974).txt',encoding='utf-8').read()
    s_vec = vectorizer.transform([s])
    svm, svm_ori = train_svm(X_train, y_train)
    
    print(svm.score(X_test, y_test))
    rs = svm.predict_proba(s_vec)
    ...
    for i in rs[0]:
        print(i)
    print(total)
    print(svm.predict(s_vec))
    print(svm_ori.score(X_test, y_test))
    
    for i, label in enumerate(classes):
        top10 = np.argsort(svm_ori.coef_[i])[-20:]
